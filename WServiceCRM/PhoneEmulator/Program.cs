...
namespace PhoneEmulator
{
    class Program
    {
        ...
        {
            var httpRequest = (HttpWebRequest)WebRequest.Create($"http://localhost:56623/cti/incomingcall?callId={callId}&callDate={callDate}&caller={caller}");
        ...
        static string LogIn()
        {
            var httpRequest = (HttpWebRequest)WebRequest.Create($"http://localhost:56623/cti/login?inputNumber={shortNumberOfSubscriber}");
            httpRequest.Method = "POST";
            httpRequest.ContentLength = 0;
            httpRequest.ContentType = "application/json";
            try
